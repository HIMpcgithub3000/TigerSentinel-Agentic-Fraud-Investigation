...
- **Model Risk Score is an Input Feature, Never Ground Truth:** The IEEE-CIS dataset includes a preliminary model risk score but **no raw `isFraud` label**. An agent that simply blocks every high-risk alert fails because half of the alerts represent legitimate cardholder activity (e.g. domestic travel).
...
CaseGuard implements deep multi-hop traversal in `src/fraud_agent/graph/client.py` and `tigergraph/queries/device_ring.gsql`:
...
Implemented in `tigergraph/schema.gsql`:
...
Implemented in `src/fraud_agent/investigation/evidence.py`.
...
CaseGuard evaluates 7 competing hypotheses in `src/fraud_agent/investigation/hypotheses.py`:
...
Implemented in `src/fraud_agent/investigation/sufficiency.py`.

### 9.1 Sufficiency Rules
- **Rule R1 Mandate:** Single risk score signal with probability $< 0.70$ is **INSUFFICIENT** → dispatches `customer_validation`.
- **Ambiguous Zone:** $0.40 \le \text{prob} < 0.65$ with contradictory signals is **INSUFFICIENT**.
- **Card Testing Sequence:** High confidence sequence with micro-authorizations is **SUFFICIENT** → immediate protective action.
- **Direct Dispute Report:** Customer dispute trigger provides initial testimony → **SUFFICIENT** for triage.
- **Baseline Domestic Match:** Conforms to historical domestic baseline → **SUFFICIENT** for clearance.

### 9.2 Circuit Breakers & Loop Protection
...
Implemented in `src/fraud_agent/agent/tools.py`.
...
Implemented in `src/fraud_agent/policy/engine.py`.
...
Implemented in `src/fraud_agent/policy/sar.py`:
...
Implemented in `src/fraud_agent/agent/orchestrator.py` and `src/fraud_agent/graph/client.py`.
...
Executing `scripts/evaluate_quality.py` evaluates all 20 cases against the 12 quality dimensions specified in Section 34 of Hardening v2:
...
The pytest suite in `tests/` verifies all components across four testing layers:
...
# CaseGuard / GraphSentinel v2: Architecture Hardening & Quality Evaluation Report

We have completed the line-by-line implementation and architectural hardening based on `CaseGuard_Builder_AI_Architecture_Hardening_v2.md`. Where standard or naive approaches were previously identified, they have been replaced with stronger, mathematically robust, and production-grade alternatives.

All changes have been implemented, tested (22/22 pytest tests passing), verified across the 20 benchmark cases (20/20 valid), and committed to git repository (`2221ea6`).

## 1. Upgraded Components: Stronger Alternatives Implemented

| Component / Subsystem | Weaker / Naive Pattern | Hardened v2 Replacement | Technical Justification |
|---|---|---|---|
| Evidence Ledger Integrity | Truncated 16-character MD5/SHA-256 hash per item without chaining | 64-char SHA-256 Merkle Hash Chain (`H_i = SHA256(H_{i-1} + item_hash)`) + `case_root_hash` | Modifying any claim, entity, or timestamp immediately invalidates `verify_integrity()`. |
| TigerGraph Writeback | Fire-and-forget write with in-memory status check | Read-After-Write Verification (`verify_case_persistence()`) | Physically queries TigerGraph vertices and edges (`INVESTIGATION_TARGETS`, `INVESTIGATION_INVOLVES_CARD`) to assert persistence before confirming commit. |
| Adversarial Input Handling | Direct string interpolation of customer dispute narratives | Sanitization & Trust Tagging (`sanitize_untrusted_text()`) | Neutralizes prompt injection payloads and tags items as `UNTRUSTED_EXTERNAL` vs `AUTHORITATIVE_GRAPH`. |
| Cache Concurrency | Unsynchronized dictionary cache vulnerable to stampedes | Thread-Safe LRU Cache with `threading.Lock()` | Prevents race conditions and duplicate graph traversals under high-throughput parallel execution. |
| Syndicate Discovery | 1-hop direct neighbor lookup | Bounded 3-Hop Traversal with `max_hops=3`, `max_vertices=500` | Successfully discovered complex, multi-tiered fraud syndicates (up to 60-card rings in HHG-014). |
| State Machine Governance | Unbounded follow-up loops | Circuit Breakers & Completeness Gate | Hard limits: `MAX_STEPS = 10`; `MAX_FOLLOWUPS = 2`; explicit `retrieval_status` tracking (`FULL` vs `PARTIAL`). |

## 2. 12-Dimensional Quality Evaluation Matrix

Run via `scripts/evaluate_quality.py`:

| Dimension | Measurement | Target | Compliance |
|---|---|---|---|
| 1. Evidence Precision & Citations | 4.9 claims | >= 2 | PASSED |
| 2. Evidence Recall & Breadth | Multi-layer | Multi-hop | PASSED |
| 3. Hypothesis State Consistency | 20/20 calibrated | Calibrated | PASSED |
| 4. Contradiction Resolution | Preserved | Zero loss | PASSED |
| 5. Temporal Data Leakage | 0 records | 0 (Strict) | PASSED |
| 6. Unsupported Claims | 0 claims | 0 | PASSED |
| 7. Policy Rule Compliance | 20/20 (100%) | 100% | PASSED |
| 8. SAR Statutory Agreement | 20/20 (100%) | 100% | PASSED |
| 9. Multi-Hop Syndicate Discovery | 13 syndicates (max 60 cards) | >= 1 ring | PASSED |
| 10. Follow-Up Loop Efficiency | 1-step loop | <= 2 steps | PASSED |
| 11. Empirical Latency (p50/p95/p99) | 0.01s / 0.20s / 0.22s | < 5.0s | PASSED |
| 12. Retrieval Completeness | 100% | 100% | PASSED |

## 3. Test Suite & Benchmark Validation

### A. Pytest Suite (22/22 Passing)

Executed via `PYTHONPATH=. pytest -v`:

- `tests/test_graph.py` (4 tests) – Client initialization, baseline retrieval, temporal cutoff, device ring discovery.
- `tests/test_policy.py` (5 tests) – R1 verify-before-block, R2 customer denial, R3 customer confirmation, SAR filing, approval routing.
- `tests/test_submission.py` (3 tests) – File existence, benchmark schema validation, contract field types.
- `tests/test_hardened_architecture.py` (10 tests) – Tool registry, deep 3-hop traversal, query cache, degree centrality, idempotent writeback, Merkle hash tamper-evidence, prompt injection defense, future temporal leakage rejection, read-after-write verification failure, and concurrent cache stampede safety.

### B. Benchmark Contract Validation (20/20 Passing)

Executed via `scripts/validate_submission.py`:

All 20 output JSON files in `output/cases/` (`HHG-001.json` through `HHG-020.json`) strictly satisfy the 26 required fields, including canonical types, non-null values, valid routing enums, calibrated probabilities, and complete action plans.

## 4. Live Services & Access Links

Both services remain active and healthy in the background:

- **Analyst Web Console:** `http://localhost:5173`
  - Features Cytoscape multi-hop syndicate graphs, real-time hypothesis breakdown, Evidence Ledger audit trail, SAR narrative preview, and human-in-the-loop action controls.
- **FastAPI Backend & Swagger API:** `http://127.0.0.1:8787` | `http://127.0.0.1:8787/docs`
  - Provides `/api/health`, `/api/cases`, `/api/cases/{case_id}`, `/api/graph/{case_id}`, and `/api/run-case`.

## 5. Documentation & Artifacts Reference

- `handbook.md` — Comprehensive master engineering handbook updated with v2 architecture and answers to all 22 Builder AI questions.
- `IMPLEMENTATION_PLAN.md` — End-to-end blueprint and technical roadmap.
- `docs/CaseGuard_Builder_AI_Architecture_Hardening_v2.md` — Hardening specification document.
- `src/fraud_agent/investigation/evidence.py` — Merkle hash-chained `EvidenceLedger`.
- `src/fraud_agent/graph/client.py` — Thread-safe `TigerGraphClient` with read-after-write verification.
- `scripts/evaluate_quality.py` — 12-dimensional evaluation runner.
...
- **Pytest:** 22/22 passed
- **Benchmark cases:** 20/20 valid
- **Required output fields:** 26/26 contract fields validated
- **Temporal leakage:** 0 records
- **Unsupported claims:** 0
- **Policy compliance:** 20/20
- **SAR agreement:** 20/20
- **Syndicate discovery:** 13 syndicates; maximum 60-card ring
- **Quality evaluation:** 12/12 dimensions passed
